...
public class EditDocumentController{// implements ServletContextAware{
        ...
        */ 
        ...
        try{
            ...
            if(!ServletFileUpload.isMultipartContent(req)){
                ...
                {
                    String docIdStr = req.getParameter("id");
                    if(docIdStr!=null && docIdStr.trim().length()>0){
                        docId = Long.parseLong(docIdStr);
                        doc.setId(docId);
                        Query q = entityManager.createQuery("SELECT f FROM File f WHERE f.document = :doc");
                    ...
                    if (item.isFormField()) {
                        ...
                        if("domain".equals(name)){
                            ...
                            if("year".equals(name)){
                                ...
                                if(s.length()!=0)
                                    doc.setResolution(Integer.parseInt(s));
                            }else if("projectionCode".equals(name)){
                            ...
                            if(realFile.exists()){
                                if(wereFiles!=null){
                                    for(File fff : wereFiles){
                                        String filePath = realFile.getAbsolutePath().substring(getRootPath(req).length());
                                        if(filePath.equals(fff.getPath())){
                                            cantDelete = false;
                                            realFile.delete();
                                            break;
                                        }
                ...
                // find which filed were replaced
                Set<File>preserve = new HashSet<File>(3);
                for(File fff: unchangedFiles){
                    fff = entityManager.merge(fff);
                    loadingFiles.add(fff);
                    
                    for(File ffff: wereFiles){
                        if(ffff.getPath().equals(fff.getPath()))
                            preserve.add(fff);
                    }
                }
                for(File toDel: wereFiles){
                    if(!preserve.contains(toDel)){
                        java.io.File delFile = new java.io.File(getRootPath(req), toDel.getPath());
                        delFile.delete();
                        entityManager.remove(toDel);
                    }
                }// teach me better
                
                //loadingFiles.addAll(unchangedFiles);
                doc.setFiles(loadingFiles);
